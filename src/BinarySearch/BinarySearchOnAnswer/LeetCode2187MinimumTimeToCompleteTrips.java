...
*/
public class LeetCode2187MinimumTimeToCompleteTrips {
    /*
    class Solution {
    public boolean isPossible(long timeGiven, int[] time, int totalTrips){
        long trips = 0;
        for(int i=0; i<time.length; i++){
            trips  += timeGiven/time[i];
        }
        return trips >= totalTrips;
    }
    public long minimumTime(int[] time, int totalTrips) {
    long maxTime = 0;
    for(int ele : time) maxTime = Math.max(maxTime,ele);
    long low = 1;
    long high = maxTime * totalTrips;
    while(low<=high){
        long mid = low + (high-low)/2;
        if(isPossible(mid,time,totalTrips)) high = mid-1;
        else low =  mid+1;
    }
    return low;
    }
