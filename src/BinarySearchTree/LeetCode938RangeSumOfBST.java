...
class Solution {
    public int Sum(TreeNode root,int low, int high){
        ...
        if(root.val<low) sum += Sum(root.right,low,high);
        else if(root.val>high) sum += Sum(root.left,low,high);
        else{
...
class Solution {
    public int Sum(TreeNode root, int low, int high) {
        if (root == null) return 0;
        if (root.val < low)
            return Sum(root.right, low, high);  // Skip left subtree
        if (root.val > high)
            return Sum(root.left, low, high);  // Skip right subtree
